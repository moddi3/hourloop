...
class Modify extends Component {
  ...
  isValid = () => {
    if (
      moment(this.state.lesson.startsAt, 'H:mm').isAfter(moment(this.state.lesson.endsAt, 'H:mm'))
    ) {
      ToastAndroid.show("Start time can't be greater than end time", ToastAndroid.SHORT);
      return false;
    }
    return Object.entries(this.state.lesson).every((x) => {
      if (!x[1]) {
